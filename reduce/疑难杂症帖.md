...
#### transform属性
transform属性作为css3新增属性，会对元素带来如下限制
- 会提升元素z轴的层叠顺序
- 如父级元素存在transform属性，子元素position: fixed则表现为position: absolute
- transform元素会限制absolute元素的宽度，换句话话，设置了transform的元素可以作为absolute元素的包含块
- 设置了transform的元素且设置overflow: hidden元素，会隐藏absolute元素溢出部分
